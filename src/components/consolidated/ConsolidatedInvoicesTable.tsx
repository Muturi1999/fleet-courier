  ...
  return (
    <>
      <div className="table-wrap border-0">
        <table className="data-table text-xs" style={{ minWidth: 920 }}>
          <thead>
            <tr>
              <th>Serial</th>
              <th>Vehicle</th>
            ...
            {loading ? (
              ...
              rows.map((inv) => (
                ...
                >
                  ...
                  <td className="max-w-[140px] truncate" title={formatPeriodRange(inv.periodStart, inv.periodEnd)}>
                  ...
                  <td
                    className="max-w-[120px] truncate text-xs text-fleet-gray-400"
                    title={describePaymentCountdown(inv) ?? undefined}
                  >
                    {describePaymentCountdown(inv) ?? "—"}
                  </td>
                  <td className="sticky right-0 z-[1] bg-white shadow-[-6px_0_8px_-6px_rgba(15,23,42,0.12)] group-hover:bg-fleet-gray-50">
                    <div className="flex flex-nowrap justify-end gap-1 whitespace-nowrap">
                      <button type="button" className="btn-secondary btn-sm shrink-0 px-2" title="View" onClick={() => onView(inv.id)}>
                        <IconEye size={14} />
                      </button>
                      {onPrint && (
                        <button type="button" className="btn-secondary btn-sm shrink-0 px-2" title="Print" onClick={() => onPrint(inv.id)}>
                          <IconPrinter size={14} />
                        </button>
                      )}
                      {onDownload && (
                        <button type="button" className="btn-secondary btn-sm shrink-0 px-2" title="Download PDF" onClick={() => onDownload(inv.id)}>
                      ...
                      {(inv.status === "rejected" || inv.status === "draft") && onEdit && !inv.supersededById && (
                        <button type="button" className="btn-secondary btn-sm shrink-0 px-2" title="Revise" onClick={() => onEdit(inv.id)}>
                          <IconEdit size={14} />
                        </button>
                      )}
                      {inv.status === "draft" && (
                        <>
                          <button type="button" className="btn-accent btn-sm shrink-0 px-2" title="Share with partner" onClick={() => onShare(inv.id)}>
                            <IconSend size={14} />
                          </button>
                          <button
                            type="button"
                            className="btn-secondary btn-sm shrink-0 px-2 text-fleet-red"
                            title="Delete"
