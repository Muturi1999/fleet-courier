...
const RESOURCE = "local-deliveries";
...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  return jsonGet<LocalDelivery>(localCollection(RESOURCE), id);
}
...
export async function PUT(req: NextRequest, ctx: Ctx) {
  ...
  if (proxied) return proxied;
  return jsonUpdate<LocalDelivery>(localCollection(RESOURCE), id, body);
}
